...
if [[ -n ${database+x} ]] ;then
    ...
    docker run --rm -v odoo10_odoo-db-data-odoo10:/volume -v "$dbdir":/backup alpine sh -c "find /volume -mindepth 1 -delete ; tar -C /volume/ -xjf /backup/$dbfname"

    echo "Done."
fi

if [[ -n ${clean_table+x} ]] ;then
    docker exec "$docker_machine" /usr/bin/psql -U postgres -d odoo10 -c 'delete from ir_attachment;'
fi
